...
rcall lcd_init ; configure the lcd
ldi r18, 0 ;give the variable an initial value
...
start:
;main function
; The main function will put "-" on the lcd
; Then in the polling loop, it will check if BUTTON_IS_PRESSED is 1 or 0, and display the corresponding char in the lcd
	rjmp timer3
...
timer1:
;check if any button has been pressed if yes, change the BUTTON PRESSED to 1, if no change BUTTON PRESSED to 0
	push r22
	in r22, SREG
	push r21
	push r20
	...
	ori r16, 0x40 ; 0x40 = 0b0100 0000 ADSC, 1 when conversion is not done
	sts ADCSRA, r16

	ldi r16, low(BUTTON_SELECT_ADC)
	mov r17, r16
	ldi r16, high(BUTTON_SELECT_ADC)
	mov r18, r16 ; store BUTTON NOT PRESSED SCALAR (0x3E8), if X > 0x3E8, no button pressed
...
wait:
	lds r16, ADCSRA
	andi r16, 0x40
	brne wait ; wait until ADSC is 0, meaning conversion is done

	lds r24, ADCL
	lds r25, ADCH
	;values are loaded in X
	cp r24, r17
	cpc r25, r18
	brsh not_pressed
...
in r17, TIFR3 
sbrs r17, OCF3A ; keep waiting if OCF3A bit in TIFR3 is set, meaning top value held in OCR3A has not been reached
rjmp timer3

ldi r17, 1<<OCF3A ; clears it by the logical 1 command
out TIFR3, r17

	;check if 1 or 0
	lds r18, BUTTON_IS_PRESSED
	cpi r18, 0 ;if 0 - NOT, 1- YES
	brne pressed_start
not_pressed_start:
	ldi r16, 1
	ldi r17, 15
